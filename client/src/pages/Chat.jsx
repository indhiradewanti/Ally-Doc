...
export default function Chat() {
  ...
  let options = {
    ...
    token: null,
    uid: isLogged === "user" ? String(filtered.userId) : String(filtered.doctorId), // "test",
  };
        ...
        <div className="flex flex-row new-h w-full overflow-x-hidden">
          <div className="flex flex-col py-8 pl-6 pr-2 w-80 bg-white flex-shrink-0">
            ...
            <div className="flex flex-col items-center new-bg border border-gray-200 mt-4 w-full py-6 px-4 rounded-lg">
              <img src={isLogged === "user" ? filtered.doctorPhoto : filtered.userPhoto} alt="Avatar" className="mask mask-circle h-32" />
              <div className="text-lg font-semibold mt-2">{isLogged === "user" ? filtered.doctorName : filtered.userName}</div>
              <div className="text-lg text-gray-500">{isLogged === "user" ? filtered.doctorSpecialist : filtered.userGender}</div>
              <button onClick={() => endChat()} className="btn btn-outline1 hidden sm:inline-block text-gray-700 hover:text-indigo-700 vogue font-bold w-32">
                End Chat
              </button>
            </div>
            ...
            <button className="flex flex-row items-center justify-center h-12 w-full mt-20  transition-transform transform hover:scale-110" onClick={() => handleSubmit()} disabled={isVideoCall || isCall ? true : false}>
              {!isCall && !isVideoCall ? (
                <div className="flex items-center justify-center rounded-full text-brown bg-green h-32 w-32">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-14 w-14" viewBox="0 0 20 20" fill="white">
                    ...
                    <div className="">
                      {joined ? (
                        <div className="grid grid-flow-col grid-cols-2 gap-36">
                          <div>
                            <div className="ml-2 text-3xl vogue font-bold">{filtered.userName}</div>
                          ...
                          <div>
                            <div className="ml-2 text-3xl vogue font-bold">{filtered.doctorName}</div>
                            <div className="w-24 h-2 bg-green mb-6 mx-auto"></div>
                            <div
                              id="remote-stream"
                              ref={remoteRef}
                  ...
                  <div className="flex flex-col h-full">
                    {messages.length > 1 &&
                      messages.map((mess, index) => (
                        <div className="grid grid-cols-12 gap-y-2" key={`${index}${mess}`}>
                          {mess !== "State changed to CONNECTING reason: LOGIN" &&
                            mess !== "State changed to CONNECTED reason: LOGIN_SUCCESS" &&
                            (mess.split(",")[0] === userId ? (
                              <div className="col-start-6 col-end-13 p-3 rounded-lg">
                                <div className="flex items-center justify-start flex-row-reverse">
                                  <div className="flex items-center justify-center h-10 w-10 rounded-full bg-indigo-500 flex-shrink-0">{isLogged === "user" ? filtered.userName[0] : filtered.doctorName[0]}</div>
                              ...
                              <div className="col-start-1 col-end-8 p-3 rounded-lg">
                                <div className="flex flex-row items-center">
                                  <div className="flex items-center justify-center h-10 w-10 rounded-full bg-indigo-500 flex-shrink-0">{isLogged === "user" ? filtered.doctorName[0] : filtered.userName[0]}</div>
